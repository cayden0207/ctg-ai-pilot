...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const { user_id, expiration_at, days } = req.body || {};
    if (!user_id) return res.status(422).json({ error: 'user_id_required' });
    let nextExp: string | null = null;
    if (expiration_at) {
      nextExp = new Date(expiration_at).toISOString();
    } else if (typeof days === 'number') {
      const d = new Date();
      d.setDate(d.getDate() + days);
      nextExp = d.toISOString();
    } else {
      return res.status(422).json({ error: 'expiration_at_or_days_required' });
    }

    const { error } = await admin
      .from('profiles')
      .update({ expiration_at: nextExp })
      .eq('user_id', user_id);
    if (error) return res.status(500).json({ error: error.message });
    return res.status(200).json({ ok: true, expiration_at: nextExp });
  } catch (e: any) {
